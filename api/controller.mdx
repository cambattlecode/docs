...
Every buildable entity has `can_build_*` and `build_*` methods. All require action cooldown == 0 and sufficient resources. The `can_build_*` variants return `bool`; `build_*` returns the new entity's `int` id and raises `GameError` if not legal.

### Directional buildings

These take `(position: Position, direction: Direction)` — the direction the building faces. They return the new building's entity id.

```python
c.build_conveyor(pos, direction)          # -> int     c.can_build_conveyor(pos, direction)
c.build_splitter(pos, direction)          # -> int     c.can_build_splitter(pos, direction)
c.build_armoured_conveyor(pos, direction) # -> int     c.can_build_armoured_conveyor(pos, direction)
c.build_gunner(pos, direction)            # -> int     c.can_build_gunner(pos, direction)
c.build_sentinel(pos, direction)          # -> int     c.can_build_sentinel(pos, direction)
c.build_breach(pos, direction)            # -> int     c.can_build_breach(pos, direction)
```

### Bridge

Takes `(position: Position, target: Position)` — the bridge's output target tile (within distance² 9). Returns the new bridge's entity id.

```python
c.build_bridge(pos, target)               # -> int     c.can_build_bridge(pos, target)
```

### Non-directional buildings

Take only `(position: Position)`. They return the new building's entity id.

```python
c.build_harvester(pos)                    # -> int     c.can_build_harvester(pos)
c.build_road(pos)                         # -> int     c.can_build_road(pos)
c.build_barrier(pos)                      # -> int     c.can_build_barrier(pos)
c.build_foundry(pos)                      # -> int     c.can_build_foundry(pos)
c.build_launcher(pos)                     # -> int     c.can_build_launcher(pos)
```
...
<ResponseField name="self_destruct()" type="None">
  Destroy this unit. Builder bots deal 20 damage to their tile. Terminates the bot's turn immediately (the bot will not execute any further code this round).
</ResponseField>

<ResponseField name="resign()" type="None">
  Forfeit the game immediately. Destroys this team's core, ending the game as a loss. Terminates the bot's turn immediately.
</ResponseField>
...
<ResponseField name="spawn_builder(position: Position)" type="int">
  Spawn a builder bot on one of the 9 core tiles. Costs one action cooldown. Returns the new builder bot's entity id.
</ResponseField>
