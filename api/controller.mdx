---
title: Controller
description: Complete reference for all Controller methods available to your bot.
---

The `Controller` object is passed to your `Player.run()` method each turn. It provides all queries and actions for interacting with the game.
...
<ResponseField name="get_team(id=None)" type="Team">
  Return the team of the entity with the given id, or this unit if omitted.
</ResponseField>

<ResponseField name="get_position(id=None)" type="Position">
...
<ResponseField name="get_vision_radius_sq(id=None)" type="int">
...
<ResponseField name="get_bridge_target(id)" type="Position">
  Return the output target position of a bridge. Raises `GameError` if not a bridge.
</ResponseField>

<ResponseField name="get_stored_resource(id=None)" type="ResourceType | None">
...
<ResponseField name="get_tile_env(pos)" type="Environment">
  Return the environment type (empty, wall, ore) of the tile at pos.
</ResponseField>

<ResponseField name="get_tile_building_id(pos)" type="int | None">
  Return the id of the building on the tile at pos, or None if empty.
</ResponseField>

<ResponseField name="get_tile_builder_bot_id(pos)" type="int | None">
  Return the id of the builder bot on the tile at pos, or None if empty.
</ResponseField>

<ResponseField name="is_tile_empty(pos)" type="bool">
  Return True if the tile has no building and is not a wall.
</ResponseField>

<ResponseField name="is_tile_passable(pos)" type="bool">
...
<ResponseField name="is_in_vision(pos)" type="bool">
  Return True if pos is within this unit's vision radius.
</ResponseField>

### Nearby queries

<ResponseField name="get_nearby_tiles(dist_sq=None)" type="list[Position]">
...
<ResponseField name="get_nearby_entities(dist_sq=None)" type="list[int]">
  Return ids of all entities on tiles within dist_sq (defaults to vision radius).
</ResponseField>

<ResponseField name="get_nearby_buildings(dist_sq=None)" type="list[int]">
  Return ids of all buildings within dist_sq (defaults to vision radius).
</ResponseField>

<ResponseField name="get_nearby_units(dist_sq=None)" type="list[int]">
...
<ResponseField name="get_current_turn()" type="int">
...
<ResponseField name="move(direction)" type="None">
  Move this builder bot one step in direction. Raises `GameError` if not legal.
</ResponseField>

<ResponseField name="can_move(direction)" type="bool">
...
These take `(position, direction)` — the direction the building faces:
...
c.build_launcher(pos, direction)          c.can_build_launcher(pos, direction)
```

### Bridge

Takes `(position, target)` — the bridge's output target tile (within distance² 9):
...
<ResponseField name="heal(position)" type="None">
  Heal all friendly entities on the tile by 10 HP. Costs one action cooldown.
</ResponseField>

<ResponseField name="can_heal(position)" type="bool">
  Return True if this builder bot can heal the tile this turn.
</ResponseField>

<ResponseField name="destroy(building_pos)" type="None">
  Destroy the allied building at building_pos. Does **not** cost action cooldown.
</ResponseField>

<ResponseField name="can_destroy(building_pos)" type="bool">
...
<ResponseField name="place_marker(position, value)" type="None">
  Place a marker with the given u32 value. Does not cost action cooldown. Max one per turn.
</ResponseField>

<ResponseField name="can_place_marker(position)" type="bool">
  Return True if this unit can place a marker at position this turn.
</ResponseField>

<ResponseField name="get_marker_value(id)" type="int">
  Return the u32 value stored in the friendly marker.
</ResponseField>

## Combat

<ResponseField name="fire(target)" type="None">
  Fire this turret at target. Use `launch()` for launchers.
</ResponseField>

<ResponseField name="can_fire(target)" type="bool">
  Return True if this turret can fire at target this turn.
</ResponseField>

<ResponseField name="launch(bot_pos, target)" type="None">
  Pick up the builder bot at bot_pos and throw it to target.
</ResponseField>

<ResponseField name="can_launch(bot_pos, target)" type="bool">
  Return True if this launcher can pick up and throw the bot.
</ResponseField>

## Core

<ResponseField name="spawn_builder(position)" type="None">
  Spawn a builder bot on one of the 9 core tiles. Costs one action cooldown.
</ResponseField>

<ResponseField name="can_spawn(position)" type="bool">
  Return True if the core can spawn a builder at position this turn.
</ResponseField>

## Debug indicators

<ResponseField name="draw_indicator_line(pos_a, pos_b, r, g, b)" type="None">
  Draw a debug line between two positions with RGB colour. Saved to the replay.
</ResponseField>

<ResponseField name="draw_indicator_dot(pos, r, g, b)" type="None">
