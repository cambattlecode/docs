...
<Warning>
  ...
  These are called **walkable** tiles.
</Warning>
...
Destroy any allied building within action radius. This can be done **any number of times per turn** and does **not** cost action cooldown.
...
Builder bots (like all units) can place one [marker](/spec/other-buildings#marker) per turn within action radius, separate from the action cooldown.
