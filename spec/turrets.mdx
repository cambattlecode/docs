...
Every turret faces in one of **8 directions**. Ammo must be fed via conveyors from any direction **except** the direction the turret faces (diagonal turrets accept from all four sides).

A turret can hold up to one stack of one resource type and only accepts incoming resources when completely empty.
...
Picks up and **throws adjacent builder bots** to a target tile within range. The target tile must be bot-passable.
