...
The [ladder](https://game.battlecode.cam/ladder) ranks all teams by Glicko-2 rating. Every 10 minutes, the scheduler creates one match per team, pairing you with a similarly-rated opponent. Matches are **best-of-5** — the first team to win 3 games wins the match. Ratings update after each scheduler cycle. See [How matches work](/getting-started/matches) for details.
