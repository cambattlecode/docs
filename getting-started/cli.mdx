---
title: CLI reference
description: Every command available in the cambc CLI.
---
...
The starter bot demonstrates core gameplay: the core spawns builder bots, builders explore laying conveyors, and when they find titanium ore they build harvesters on it.
...
| `--seed N` | Map seed for deterministic generation (overrides `cambc.toml` default) |
...
Show your current team, rating, and member list.
...
Both bots are packaged and uploaded to the server. Unlike `cambc run`, this enforces the 2ms CPU time limit per turn — use this to check your bot's performance before submitting.
