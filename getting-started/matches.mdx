...
description: How ladder matches, ratings, and the best-of-5 format work.
---

## Match format

Every match is a **best-of-5** series. The first team to win 3 games wins the match. Each game in the series is played on a different map with a different random seed.
...
Ratings are updated using **Glicko-2** after each scheduler cycle. Match outcomes use fractional scoring based on the game score (e.g., a 3-1 win counts more than a 3-2 win).
...
<Warning>
  Rate limits apply to both unrated and test matches: max 10 per 5 minutes, plus a 5-minute cooldown per specific matchup (unrated only).
</Warning>
